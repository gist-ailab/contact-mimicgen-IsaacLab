...
try:
    from isaaclab.devices.spacemouse.se3_pyspacemouse import Se3PySpaceMouse
except Exception:
    Se3PySpaceMouse = None
try:
...
from assembly.bolt_nut.config.franka import boltnut_ik_rel_env_cfg
from assembly.bolt_nut.config.franka import agents
...
#     kwargs={
#         "env_cfg_entry_point": boltnut_ik_rel_env_cfg.FrankaBoltNutEnvCfg,
...
def pre_process_actions(
    ...
    if "Reach" in args_cli.task:
        ...
        # Normalize teleop_data into (delta_pose, gripper_command)
        delta_pose = None
        gripper_command = False
        if isinstance(teleop_data, (list, tuple)):
            if len(teleop_data) == 2 and isinstance(teleop_data[1], (bool, np.bool_)):
                delta_pose, gripper_command = teleop_data
            elif len(teleop_data) >= 2 and isinstance(teleop_data[-1], (bool, np.bool_)):
                gripper_command = teleop_data[-1]
                delta_pose = np.asarray(teleop_data[0]).reshape(-1)
            else:
                delta_pose = np.asarray(teleop_data[0]).reshape(-1)
        else:
            delta_pose = np.asarray(teleop_data).reshape(-1)
        # ensure delta_pose is exactly 6D (x, y, z, rx, ry, rz)
        delta_pose = np.asarray(delta_pose).reshape(-1)
        if delta_pose.shape[0] > 6:
            delta_pose = delta_pose[:6]
        elif delta_pose.shape[0] < 6:
            pad = np.zeros(6 - delta_pose.shape[0], dtype=delta_pose.dtype)
            delta_pose = np.concatenate([delta_pose, pad], axis=0)
        # convert to torch
        delta_pose = torch.tensor(delta_pose, dtype=torch.float, device=device).repeat(num_envs, 1)
        ...
        if not osc:
            ...
            if stiffness.shape[0] == 1:
                return torch.concat([delta_pose, stiffness, gripper_vel], dim=1)
            else:
...
def main():
    ...
    def create_teleop_device(device_name: str, env: gym.Env):
        ...
        if device_name == "keyboard":
            # Prefer cfg-style construction if available (newer API)
            if Se3KeyboardCfg is not None:
                try:
                    kb_cfg = Se3KeyboardCfg()
                    if hasattr(kb_cfg, "pos_sensitivity"):
                        kb_cfg.pos_sensitivity = 0.2
                    if hasattr(kb_cfg, "rot_sensitivity"):
                        kb_cfg.rot_sensitivity = 0.5
                    return Se3Keyboard(kb_cfg)
                except Exception:
                    pass
            # Fallback to legacy constructors
            try:
                return Se3Keyboard(0.2, 0.5)
            except Exception:
                kb = Se3Keyboard()
                if hasattr(kb, "pos_sensitivity"):
                    kb.pos_sensitivity = 0.2
                if hasattr(kb, "rot_sensitivity"):
                    kb.rot_sensitivity = 0.5
                return kb
        elif device_name == "spacemouse":
            sm_cfg = Se3SpaceMouseCfg()
            sm_cfg.pos_sensitivity = 0.2
            sm_cfg.rot_sensitivity = 0.5
            try:
                return Se3SpaceMouse(sm_cfg)
            except Exception as e:
                if Se3PySpaceMouse is not None:
                    print(f"[teleop] HID SpaceMouse open failed: {e}. Falling back to PySpaceMouse.")
                    omni.log.warn(f"Failed to open HID SpaceMouse ({e}). Falling back to PySpaceMouse.")
                    return Se3PySpaceMouse(pos_sensitivity=0.1, rot_sensitivity=0.2)
                else:
                    print(f"[teleop] HID SpaceMouse open failed: {e}. PySpaceMouse unavailable; falling back to Keyboard.")
                    omni.log.warn(f"Failed to open HID SpaceMouse ({e}). PySpaceMouse unavailable; falling back to Keyboard.")
                    if Se3KeyboardCfg is not None:
                        try:
                            kb_cfg = Se3KeyboardCfg()
                            if hasattr(kb_cfg, "pos_sensitivity"):
        ...
        # elif device_name == "pyspacemouse":
            # return Se3SpaceMouse(pos_sensitivity=0.1, rot_sensitivity=0.2)
    ...
    # set visualization markers for ee frame
    vis_marker_cfg = FRAME_MARKER_CFG.copy()
    vis_marker_cfg.markers["frame"].scale = (0.05, 0.05, 0.05)
    vis_marker_cfg.markers["frame"].visual_material = sim_utils.PreviewSurfaceCfg(
        diffuse_color=(0.0, 0.0, 0.0),
        opacity=0.05,
    )
    vis_marker_cfg.prim_path = "/Visuals/FrameTransformerVis"
    vis_marker = VisualizationMarkers(vis_marker_cfg)
    
    # set stiffness for osc controller
    stiffness = torch.tensor([320.0, 320.0, 320.0, 400.0, 400.0, 400.0], dtype=torch.float, device=env.device)
